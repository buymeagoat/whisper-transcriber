...
Another script `scripts/docker_build.sh` prunes Docker resources and then rebuilds the images and stack from scratch. Both `docker_build.sh` and `update_images.sh` stage Docker images and Python/Node packages before any build step so network hiccups do not interrupt the process. `update_images.sh` inspects the running containers and rebuilds a service only when its image is missing or the container reports an unhealthy status. These scripts source `scripts/shared_checks.sh` which ensures Whisper models and `ffmpeg` are present, `.env` contains a valid `SECRET_KEY`, and the `uploads`, `transcripts` and `logs` directories exist. It also checks that **Node.js 18+** is installed and that the `docker compose` command is available. `docker_build.sh` also verifies that the `whisper-transcriber-api` and `whisper-transcriber-worker` images were created successfully before starting the stack. Once running, access the API at `http://localhost:8000`.
Run `scripts/prestage_dependencies.sh` on a networked host to download all required Docker images and packages into `cache/`. Execute this helper before `scripts/docker_build.sh`, `scripts/update_images.sh` or `scripts/start_containers.sh` when preparing an offline build.
Set `OFFLINE_MODE=true` in the environment to force these scripts to install from the `cache/` directory even when a network connection is available.
...
Run `scripts/update_images.sh` after pulling the latest code for routine updates. The script rebuilds a service only when its Docker image is missing or the running container reports an unhealthy status, then restarts any rebuilt services. It exits early if the required Whisper models or `ffmpeg` are not present.
