...
The build scripts default to `/tmp/docker_cache` for storing cached packages and images. When running under Windows Subsystem for Linux (WSL) and `CACHE_DIR` is unset, they use `/mnt/c/whisper_cache` instead. Create that directory if it does not already exist or set `CACHE_DIR` to override the location.
