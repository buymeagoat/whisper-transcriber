...
Set `CACHE_DIR` to a Windows mount (for example `/mnt/c/whisper-cache`) before running these scripts so Windows can inspect the cache while Docker saves images without permission errors.
