...
Run `scripts/prestage_dependencies.sh` on a networked host to download all required Docker images and packages into `cache/`. The script now verifies Docker is running with `docker info` and checks for `cache/images`, `cache/pip`, `cache/npm` and the optional `cache/apt` directories, reporting any missing assets. Execute this helper before `scripts/docker_build.sh`, `scripts/update_images.sh` or `scripts/start_containers.sh` when preparing an offline build.
