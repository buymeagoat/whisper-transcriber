...
Run `scripts/check_env.sh` before offline builds to verify DNS resolution, confirm cached `.deb` archives match the Dockerfile base image and warn when the cached digest differs from the current pull. If WSL2 DNS causes timeouts, set `DNS_SERVER=<ip>` or build with `--network=host` so Docker can reach the registry.
