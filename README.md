...
Run `scripts/prestage_dependencies.sh` on a networked host to download all required Docker images and packages into `cache/`. Execute this helper before `scripts/docker_build.sh` or `scripts/update_images.sh` when preparing an offline build.
