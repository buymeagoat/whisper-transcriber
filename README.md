...
Run `scripts/update_images.sh` after pulling the latest code for routine updates. The script rebuilds a service only when its Docker image is missing or the running container reports an unhealthy status, then restarts any rebuilt services.
