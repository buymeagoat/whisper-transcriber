...
For auditing purposes the prestage script also writes the wheel filenames to `cache/pip/pip_versions.txt`, lists installed Node packages in `cache/npm/npm_versions.txt` and archives the npm cache as `cache/npm/npm-cache.tar`.
