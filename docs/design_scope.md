...
- `scripts/` – packaging helpers that generate `dist/whisper-transcriber.exe` and `dist/whisper-transcriber.rpm`.
 ...
 - `update_images.sh` – rebuilds the API and worker images using Docker's cache and restarts those services without pruning existing resources. Like `docker_build.sh` it stages dependencies up front. Its log is written to `logs/update_images.log`. Run it after `git fetch` and `git pull` for routine code updates.
 - `prestage_dependencies.sh` – download Docker images and Python/Node packages into `cache/` for offline use. Run this before `docker_build.sh`, `update_images.sh` or `start_containers.sh` on a networked machine.
