...
- `scripts/` – packaging helpers that generate `dist/whisper-transcriber.exe` and `dist/whisper-transcriber.rpm`.
 ...
 - `prestage_dependencies.sh` – download Docker images and Python/Node packages into `cache/` for offline use. Run this before `docker_build.sh` or `update_images.sh` on a networked machine.
