...
- The build scripts store cached packages and Docker images under `/tmp/docker_cache` by default. When running under WSL and `CACHE_DIR` is not set they automatically use `/mnt/c/whisper_cache`. Create that directory if needed or set `CACHE_DIR` to use a custom path.
