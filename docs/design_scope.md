...
- `prestage_dependencies.sh` – wipe the `cache/` directory and download all Docker images and Python/Node packages needed for the build. The build scripts automatically run this step, so invoke it manually only when you want a separate download pass. It aborts early if Node.js 18 or newer is not available.
