...
- `prestage_dependencies.sh` – download Docker images and Python/Node packages into `cache/` for offline use. The build scripts automatically run this step, so invoke it manually only when you want a separate download pass. It aborts early if Node.js 18 or newer is not available.
