...
| R2 | Allow the single administrator to exchange the bootstrap credentials for a JWT (multi-user login deferred). | `api/routes/auth.py::login`, [docs/api/routes.md](api/routes.md#post-authlogin) | `tests/test_api_smoke.py::test_admin_login_returns_token_and_cookie` |
